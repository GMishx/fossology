...
class Scanners:
  ...
  def __get_ojo_result(self, dir_to_scan: str) -> dict:
    ...
    """
    ojo_process = Popen([self.ojo_path, "-J", "-d", dir_to_scan],
                        stdout=PIPE)
  ...
  def set_copyright_list(self, all_results: bool = False, whole: bool = False) -> None:
    """
    ...
    if not self.cli_options.scan_only_deps:
      self.scan_packages.parent_package['COPYRIGHT_RESULT'] = self.__process_single_copyright_package(
        component=self.scan_packages.parent_package,
        is_parent=True,
  ...
  def __process_single_copyright_package(
    ...
    if is_parent:
      ...
      dir_to_scan = os.path.join(component['download_dir'], component['base_dir'])
  ...
  def set_keyword_list(self, whole:bool = False) -> None:
    ...
    """
    if not self.cli_options.scan_only_deps:
      self.scan_packages.parent_package['KEYWORD_RESULT'] = self.__process_single_keyword_package(
        component=self.scan_packages.parent_package,
        is_parent=True,
        whole=whole
      )
  ...
  def __set_license_nomos(self, whole: bool = False) -> None:
    ...
    """
    if not self.cli_options.scan_only_deps:
      self.scan_packages.parent_package['NOMOS_RESULT'] = self.__process_single_nomos_package(
        component=self.scan_packages.parent_package,
        is_parent=True,
        whole=whole
      )
  ...
  def __process_single_nomos_package(self, component: dict, is_parent: bool, whole: bool) -> Union[List[ScanResult],List[ScanResultList]]:
    """
  ...
  def __set_license_ojo(self, whole:bool = False) -> None:
    ...
    """
    if not self.cli_options.scan_only_deps:
      self.scan_packages.parent_package['OJO_RESULT'] = self.__process_single_ojo_package(
        component=self.scan_packages.parent_package,
        is_parent=True,
        whole=whole
      )
  ...
  def __process_single_ojo_package(self, component: dict, is_parent: bool, whole: bool) -> Union[List[ScanResult],List[ScanResultList]]:
    """
  ...
  def set_scanner_results(self, whole:bool = False) -> None:
    ...
    if self.cli_options.nomos and self.cli_options.ojo:
      if whole:
        self.scan_packages.parent_package['SCANNER_RESULTS'] = self.scan_packages.parent_package.get('NOMOS_RESULT', []) + self.scan_packages.parent_package.get('OJO_RESULT', [])
        for purl in self.scan_packages.dependencies.keys():
          component = self.scan_packages.dependencies[purl]
          component['SCANNER_RESULTS'] = component.get('NOMOS_RESULT', []) + component.get('OJO_RESULT', [])
      else:
        self.scan_packages.parent_package['SCANNER_RESULTS'] = self.__merge_nomos_ojo(self.scan_packages.parent_package.get('NOMOS_RESULT', []), self.scan_packages.parent_package.get('OJO_RESULT', []))
        for purl in self.scan_packages.dependencies.keys():
          component = self.scan_packages.dependencies[purl]
          component['SCANNER_RESULTS'] = self.__merge_nomos_ojo(component.get('NOMOS_RESULT', []), component.get('OJO_RESULT', []))
    else:
      scanner_key = 'NOMOS_RESULT' if self.cli_options.nomos else 'OJO_RESULT'
      self.scan_packages.parent_package['SCANNER_RESULTS'] = self.scan_packages.parent_package.get(scanner_key, [])
      for purl in self.scan_packages.dependencies.keys():
        component = self.scan_packages.dependencies[purl]
