...
function getClearedLicensesForDump(DbManager $dbManager, ItemTreeBounds $itemTreeBounds, $whiteListedGroupIDs = array())
{
              ...
              FROM clearing_decision cd
                INNER JOIN $uploadTreeTable ut
                  ON (ut.pfile_fk = cd.pfile_fk AND cd.scope = $globalScope)
                    OR (ut.uploadtree_pk = cd.uploadtree_fk AND cd.scope = $localScope $other_clauses)
              WHERE $sql_upload $condition
                AND cd.decision_type!=$WIP AND cd.decision_type!=$IRR
              ORDER BY itemid, scope, id DESC
              )";
    ...
    {
        $fileNameInRepo = RepPath($row['file_id']);
        $matches[] =[
          'source' => 'CONCLUSION',
          'path' => $fileNameInRepo,
...
function dumpAllFiles($mongoManager)
{
  ...
  while ($row = $dbManager->fetchArray($result))
  {
    $fileNameInRepo = RepPath($row['pfile_pk']);
