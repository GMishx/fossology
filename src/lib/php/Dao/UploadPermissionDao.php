...
class UploadPermissionDao extends Object
{
  ...
  /**
   * Get the list of groups which can access the given upload
   * @param int $uploadId UploadID for which the groups are required
   * @param int $perm Groups having permissions more than $perm
   * @param boolean $string If true, returns groups in a string with ',' as a delimiter else as an array
   * @return int[] List of groups which can access the upload
   */
  public function getAccessibleGroups($uploadId, $perm = Auth::PERM_NONE, $string = TRUE)
  {
    if(!in_array($perm, array(Auth::PERM_ADMIN, Auth::PERM_NONE, Auth::PERM_READ, Auth::PERM_WRITE)))
    {
      $perm = Auth::PERM_NONE;
    }
    $this->dbManager->prepare($stmt=__METHOD__,
      "SELECT group_fk FROM perm_upload WHERE upload_fk=$1 AND perm > $perm");
    $res = $this->dbManager->execute($stmt, array(intval($uploadId)));
    $groups = array();
    while($row=$this->dbManager->fetchArray($res))
    {
      $groups[] = $row['group_fk'];
